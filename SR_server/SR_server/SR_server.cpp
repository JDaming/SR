...
int waitCount[SEQ_SIZE];//对每一个分组进行定时
int curSeq;//当前数据包的 seq
int curAck;//当前等待确认的 ack
int totalSeq;//收到的包的总数
int totalPacket;//需要发送的包总数
long FileSize;//文件大小
int SentTotalPacket;//已发送包的数量
//int totalAck;//收到的ACK总量
std::ifstream icin;
char data[1024 * SEQ_SIZE];//设置缓冲区
//************************************
// Method: getCurTime
// FullName: getCurTime
// Access: public
...
// Access: public
// Returns: void
// Qualifier: 超时重传处理函数，重传需要重传的数据seq
//************************************
void timeoutHandler(SOCKET sockServer, SOCKADDR_IN addrClient,char *data) {
	char buffer[BUFFER_LENGTH];
	printf("Timer out error.\n");
	printf("Start Resend the seq:%d\n", curAck);
	buffer[0] = curAck + 1;
	ack[curAck] = FALSE;
	//数据发送的过程中应该判断是否传输完成
	//为简化过程此处并未实现
	memcpy(&buffer[1], data + 1024 * (curAck), 1024);
...
void ackHandler(char c) {
	...
	if (index == curAck) {
		while (ack[curAck] && curAck != curSeq) {
			icin.read(data + curAck * 1024, 1024);
			curAck++;
			curAck %= SEQ_SIZE;
		}
	}
...
int main(int argc, char* argv[])
{
	...
	//将测试数据读入内存
	icin.open(filepath);
	icin.seekg(0, std::ios::end);
	FileSize = icin.tellg();
	icin.seekg(0, std::ios::beg);
	ZeroMemory(data, sizeof(data));
	icin.read(data, 1024 * SEQ_SIZE);
	totalPacket = FileSize / 1024;
	int recvSize;
	
	while (true) {
		//非阻塞接收，若没有收到数据，返回值为-1
		recvSize =
			recvfrom(sockServer, buffer, BUFFER_LENGTH, 0, ((SOCKADDR*)&addrClient), &length);//从（已连接）套接口上接收数据，并捕获数据发送源的地址。（屌屌屌）
		if (recvSize < 0) {
			Sleep(200);
			continue;
		}
			...
			while (runFlag) {
				...
				case 0://发送 205 阶段
					...
					sendto(sockServer, buffer, strlen(buffer) + 1, 0,
						(SOCKADDR*)&addrClient, sizeof(SOCKADDR));//指向一指定目的地发送数据，sendto()适用于发送未建立连接的UDP数据包 
					Sleep(100);
					stage = 1;
					break;
				case 1://等待接收 200 阶段，没有收到则计数器+1，超时则放弃此次“连接”，等待从第一步开始
					...
					break;
				case 2://数据传输阶段
					if (seqIsAvailable() && SentTotalPacket<totalPacket) {
						//发送给客户端的序列号从 1 开始
						buffer[0] = curSeq + 1;
						ack[curSeq] = FALSE;
						//数据发送的过程中应该判断是否传输完成
					...
					if (waitCount[curAck] > 20)//20 次等待 ack 则超时重传
					{
						//resendSeq = curAck;
						timeoutHandler(sockServer, addrClient, data);
